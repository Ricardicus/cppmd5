...
static void byteReverse(unsigned char *buf, unsigned longs) {
  do {
    uint32 t = static_cast<uint32 >((static_cast<unsigned>(buf[3] << 8 | buf[2]) << 16) |
      (static_cast<unsigned>(buf[1] << 8 | buf[0])));
    *reinterpret_cast<uint32 *>(buf) = t;
    buf += 4;
  } while (--longs);
}
#endif
...
void MD5Hasher::MD5Update(unsigned char *buf, unsigned len) {
  ...
  if (t) {
    unsigned char *p = reinterpret_cast<unsigned char *>(ctx.in + t);
    ...
    MD5Transform(ctx.buf, reinterpret_cast<uint32 *>(ctx.in));
    buf += t;
    len -= t;
  }
...
std::string MD5Hasher::MD5Final() {
  ...
  *p++ = 0x80;
  ...
  (reinterpret_cast<uint32 *>(ctx.in))[14] = ctx.bits[0];
  (reinterpret_cast<uint32 *>(ctx.in))[15] = ctx.bits[1];
